...
// NOTE: Le backend n'accepte que les images, mais nous laissons la validation
// de formats vidéo pour le cas où la logique du backend évoluerait.
const acceptedFormats = ['image/png', 'image/jpeg', 'image/jpg', 'image/webp']; // J'ai retiré MP4/AVI car le backend ML ne les traite pas
...
// 3. Modifier processFile pour appeler le Store après la validation
const processFile = (file: File) => {
  // Simplification de la validation pour correspondre au backend (images uniquement)
  if (!file.type.startsWith('image/')) {
    alert('Format de fichier non accepté. Veuillez choisir une image (PNG, JPG, JPEG).');
    return;
  }
  ...
  // **********************************************
  // APPEL CRITIQUE : Lancer la prédiction via le Store
  // **********************************************
  store.uploadAndPredict(file);
};

// 4. Modifier removeFile pour réinitialiser le Store
const removeFile = () => {
  ...
  // **********************************************
  // Réinitialiser les résultats de la prédiction
  // **********************************************
  store.reset();
};
</script>
